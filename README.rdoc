...
* Checked API secret into source control (on purpose)
* secret.yml for API credentials
* production set config.assets.compile = true (not part of build release)
* Have to install Postgres to deploy to Heroku?! Can't bundle install without pg_config..? bundle install --without production

== TO DO

* Global properties (e.g. items per page in controller and test)
* Deploy Heroku from Rubymine
* RSpec: pagination, modal? all tests at once, ensure all pass
* Modal popup (wasted like an hour!)
* Heroku: heroku run rails console
