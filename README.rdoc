...
Currently broken, but once fixed the running demo will be at: http://safe-ocean-7893.herokuapp.com/
...
* Possible to cache the query results until the following search (for pagination purposes), but requires persistant storage between stateless requests and therefore further investigation in a multi-user environment (cache/session, database. Performance currently acceptable.
